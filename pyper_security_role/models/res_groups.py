...
class ResGroups(models.Model):
    ...
    @api.depends('privilege_id', 'privilege_id.is_role')
    def _compute_is_role(self):
        for group in self:
            group.is_role = group.privilege_id.is_role if group.privilege_id else False

    def _inverse_is_role(self):
        for group in self:
            if group.is_role and (not group.privilege_id or not group.privilege_id.is_role):
                group.privilege_id = self.env.ref('pyper_security_role.group_security_role')
