...
class PyperImporterProvider(models.Model):
    ...
    def search_record(self, model: str, domain: list = None, context: dict = None, sudo: bool = False,
        ...
        if 'active' in env_model._fields:
            ...
            domain = Domain.AND([active_domain, domain]) if domain else active_domain
    ...
    def override_values(self, record: models.Model, vals: dict):
        record_id = record.id
        ...
        for key, value in vals.items():
            sets.append(key + "=%(" + key + ")s")
            params[key] = fields.Datetime.to_string(value) if isinstance(value, datetime) else value
    ...
    def override_log_access(self,
        ...
        self.override_values(record, params)
    ...
    @staticmethod
    def format_datetime(value: str | bool, ignoretz: bool = True, default_value=False) -> datetime | bool:
